    ...
    return (
        <section className='py-[65px] text-white px-4'>
            <h2 className='text-center lg:text-3xl md:text-2xl text-xl font-semibold font-montserrat max-w-[986px] mx-auto'>
                Never waste more than a day handing over design requirements with these features:
            </h2>
                ...
                <div className='grid grid-cols-3 xxl:gap-10 xl:gap-8 lg:gap-6 gap-4 lg:mt-[65px] mt-10'>
                    {FEATURES_GRID_DATA?.map(({ id, title, description, image }, index) => {
                        return (
                            ...
                            >
                                <div className={`rounded-3xl overflow-hidden bg-jet flex relative ${featuredGridSettings[index].cardFlexCol} ${featuredGridSettings[index].cardGradientAfter} ${featuredGridSettings[index].cardGradientBefore} justify-between items-center h-full lg:gap-y-[26px] md:gap-y-5 gap-y-4`}>
                                    <img src={image} className={`z-10 relative ${featuredGridSettings[index].cardImgPadding}`} alt="" />
